...
// 若无特殊需求，可自行定义一个简化的配置结构
typedef struct {
    i2c_mode_t mode;        // 仅支持 I2C_MODE_MASTER
    int sda_io_num;
    int scl_io_num;
    bool sda_pullup_en;
    bool scl_pullup_en;
    union {
        struct {
            uint32_t clk_speed;   // 主机时钟
        } master;
    };
    uint32_t clk_flags;     // 对应 i2c_config_t::clk_flags
} i2c_config_bus_t;

// i2c_bus 的句柄，用于对外屏蔽内部结构
...
//----------------- 创建/删除总线 -----------------//
/**
 * @brief 创建并初始化 i2c_bus
 *
 * @param port  I2C 编号(0 或 1 等)
 * @param conf  对应的主机配置
 * @return      成功则返回非 NULL，失败返回 NULL
 */
i2c_bus_handle_t i2c_bus_create(i2c_port_t port, const i2c_config_bus_t *conf);
 ...
 * @brief 删除 i2c_bus
 *
 * @param p_bus 指向要删除的总线句柄指针，函数执行完会置空
...
//----------------- 设备相关接口 -----------------//
/**
 * @brief 以给定的地址(7bit)创建一个从设备
 *
 * @param bus_handle i2c_bus_create 返回的总线句柄
 * @param dev_addr   从设备地址(7 bit)
 * @param clk_speed  若不为 0，则覆盖默认总线频率
 * @return           成功返回设备句柄，失败返回 NULL
 */
i2c_bus_device_handle_t i2c_bus_device_create(i2c_bus_handle_t bus_handle, uint8_t dev_addr, uint32_t clk_speed);

/**
 * @brief 删除某个从设备，减少 ref 计数
 *
 * @param p_dev_handle 指向要删除的从设备句柄指针，成功后置空
 * @return             ESP_OK
 */
esp_err_t i2c_bus_device_delete(i2c_bus_device_handle_t *p_dev_handle);

/**
 * @brief 获取某个从设备的 7bit 地址
 */
uint8_t i2c_bus_device_get_address(i2c_bus_device_handle_t dev_handle);
 ...
 *
 * @param bus_handle i2c_bus_create 返回的总线句柄
 * @param buf        若不为 NULL，则扫描到的地址会依次填入
 * @param num        buf 的容量(最多写多少个地址)
 * @return           实际扫描到的地址个数
 */
uint8_t i2c_bus_scan(i2c_bus_handle_t bus_handle, uint8_t *buf, uint8_t num);
